...
const ACHIEVEMENTS: AchievementDefinition[] = [
  ...
  },
  {
    ...
    description: 'Perform 10,000 total rolls.',
  ...
  // Credit Milestones
  {
    ...
    check: ({ stats }) => stats.bestRoll.gte(new Decimal(1000000)),
  },
  {
    id: 'billionaire',
    name: 'Billionaire',
    description: 'Earn at least 1 billion credits in a single roll.',
    reward: { type: 'luck_points', value: new Decimal(10) }, // 10 Luck Points
    check: ({ stats }) => stats.bestRoll.gte(new Decimal(1000000000)),
  },
  // Prestige Milestones
  {
  ...
  },
  {
    ...
    reward: { type: 'global_multiplier', value: new Decimal(0.20) }, // +20%
    check: ({ state }) => (state.prestige?.totalPrestiges ?? 0) >= 10,
  },
  ...
  // Specific Combo Milestones
  {
    id: 'royal-flush-luck',
    name: 'Royal Luck',
    description: 'Roll a Royal Flush.',
    reward: { type: 'global_multiplier', value: new Decimal(0.15) }, // +15%
    check: ({ combo }) => combo?.kind === 'flush',
  },
  {
    id: 'six-of-a-kind',
    name: 'Sextuplets',
